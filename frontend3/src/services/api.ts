...
export const api = {
  ...
  async finalizeBatch(tripId: string, batchId: string): Promise<void> {
    await fetchWithTimeout(
      `${API_BASE_URL}/api/v1/trips/${tripId}/batches/${batchId}/finalize`,
      { method: 'POST' },
      10000,
    ).catch(() => {});
  },

  async listNotifications(organizerId: string): Promise<AppNotification[]> {
    const response = await fetchWithTimeout(
      `${API_BASE_URL}/api/v1/notifications?organizer_id=${encodeURIComponent(organizerId)}`,
    );
    return handleResponse<AppNotification[]>(response);
  },

  async markNotificationRead(notificationId: string): Promise<void> {
    await fetchWithTimeout(`${API_BASE_URL}/api/v1/notifications/${notificationId}/read`, {
      method: 'PATCH',
    }).catch(() => {});
  },

  async getOrganizerSettings(organizerId: string): Promise<OrganizerSettings> {
    const response = await fetchWithTimeout(`${API_BASE_URL}/api/v1/organizers/${organizerId}/settings`);
    return handleResponse<OrganizerSettings>(response);
  },

  async updateOrganizerSettings(organizerId: string, settings: Partial<OrganizerSettings>): Promise<OrganizerSettings> {
    const response = await fetchWithTimeout(`${API_BASE_URL}/api/v1/organizers/${organizerId}/settings`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
