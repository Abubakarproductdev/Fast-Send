...
export default function TripsScreen() {
  ...
  return <ScreenShell><Animated.View style={[styles.inner, { opacity: fadeAnim }]}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.list} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadTrips(true); }} tintColor={colors.primary} />}>
      ...
      : loading ? <View style={styles.state}><Ionicons name="hourglass-outline" size={26} color={colors.primaryDark} /><Text style={styles.stateSub}>Searching archive…</Text></View>
      : trips.length === 0 ? <View style={styles.state}><View style={styles.emptyIcon}><Ionicons name={search ? 'search-outline' : 'albums-outline'} size={30} color={colors.sageDark} /></View><Text style={styles.stateTitle}>{search ? 'No matching trips' : 'No history yet'}</Text><Text style={styles.stateSub}>{search ? 'Try a different name or invite code.' : 'Your completed journeys will be collected here.'}</Text></View>
      : <>
        ...
        {hasMore && (
          <TouchableOpacity onPress={() => loadTrips(false)} disabled={loadingMore} style={styles.loadMoreBtn}>
            <Text style={styles.loadMoreText}>{loadingMore ? 'Loading...' : 'Load older trips'}</Text>
          </TouchableOpacity>
        )}
      </>}
    </ScrollView>
