...
export default function TripsScreen() {
  ...
  const loadTrips = async () => {
    ...
    setError(null);
    try { setTrips(await api.getOrganizerTrips(organizerId)); }
  ...
  const filteredTrips = useMemo(() => {
    ...
    return trips.filter((trip) => `${trip.name || 'Untitled trip'} ${trip.invite_code} ${formatDate(trip.created_at)}`.toLowerCase().includes(normalized));
  }, [search, trips]);
  ...
  return <ScreenShell><Animated.View style={[styles.inner, { opacity: fadeAnim }]}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.list} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadTrips(); }} tintColor={colors.primary} />}>
      ...
      : loading ? <View style={styles.state}><Ionicons name="hourglass-outline" size={26} color={colors.primaryDark} /><Text style={styles.stateSub}>Opening your archive…</Text></View>
      : filteredTrips.length === 0 ? <View style={styles.state}><View style={styles.emptyIcon}><Ionicons name={search ? 'search-outline' : 'albums-outline'} size={30} color={colors.sageDark} /></View><Text style={styles.stateTitle}>{search ? 'No matching trips' : 'No history yet'}</Text><Text style={styles.stateSub}>{search ? 'Try a different name, invite code, or date.' : 'Your completed journeys will be collected here.'}</Text></View>
      : filteredTrips.map((trip) => <TouchableOpacity key={trip.id} style={styles.card} activeOpacity={0.86} onPress={() => router.push(`/trip-details?tripId=${encodeURIComponent(trip.id)}`)}>
