    ...
    <ScreenShell>
      <Animated.View style={[styles.inner, { opacity: fadeAnim }]}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          <View style={styles.topBar}><TouchableOpacity onPress={() => router.back()} style={styles.backBtn}><Ionicons name="arrow-back" size={19} color={colors.textPrimary} /></TouchableOpacity><View style={styles.livePill}><Animated.View style={[styles.liveDot, { transform: [{ scale: pulseAnim }] }]} /><Text style={styles.liveText}>TRIP IS LIVE</Text></View><View style={styles.topSpacer} /></View>
          <View style={styles.header}><Text style={styles.eyebrow}>INVITE YOUR GUESTS</Text><Text style={styles.title}>Show this code to anyone at your event.</Text><Text style={styles.subtitle}>They scan it once with their phone camera — no app needed. We'll take care of the rest.</Text></View>
          <View style={styles.qrCard}>
            {tripError ? <View style={styles.errorState}><Ionicons name="alert-circle-outline" size={30} color={colors.error} /><Text style={styles.errorText}>{tripError}</Text></View> : inviteCode ? <><View style={styles.qrWrapper}><QRCode value={qrValue} size={184} color="#000000" backgroundColor="#FFFFFF" /></View><Text style={styles.inviteLabel}>OR SHARE THIS CODE</Text><Text style={styles.inviteValue}>{inviteCode}</Text></> : <View style={styles.loadingState}><Ionicons name="hourglass-outline" size={24} color={colors.primaryDark} /><Text style={styles.loadingText}>Getting your code ready…</Text></View>}
          </View>
          <View style={styles.metricsRow}><StatItem value={guests} label="GUESTS JOINED" icon="people-outline" /><View style={styles.metricDivider} /><StatItem value={photos} label="PHOTOS UPLOADED" icon="images-outline" /></View>
          <PrimaryButton title="Trip settings" type="secondary" onPress={() => activeTripId && router.push(`/trip-settings?tripId=${encodeURIComponent(activeTripId)}`)} disabled={!activeTripId || uploading || ending} style={styles.settingsButton} />
          ...
          {uploadErrors.length > 0 && <View style={styles.errorBox}><Text style={styles.errorBoxTitle}>{uploadErrors.length} photo{uploadErrors.length > 1 ? 's' : ''} failed to upload</Text>{uploadErrors.slice(0, 2).map((e, i) => <Text key={i} style={styles.errorBoxItem}>• {e}</Text>)}</View>}
          <View style={styles.actions}><PrimaryButton title={uploading ? 'Uploading...' : 'Upload my photos now'} onPress={handlePushPhotos} disabled={uploading || ending} loading={uploading} /><PrimaryButton title={ending ? 'Ending...' : 'End this trip'} type="secondary" onPress={handleEndTrip} disabled={uploading || ending} loading={ending} /></View>
        </ScrollView>
      </Animated.View>
    </ScreenShell>
  );
