...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        const synced = await api.syncOrganizer(
          firebaseUser.uid,
          firebaseUser.email,
  ...
  const setOrganizerId = async (id: string | null) => {
    ...
    try {
      await storeOrganizerId(id);
    } catch (error) {
      console.error('Failed to persist organizer session', error);
    }
  };
  ...
  const setActiveTripId = async (id: string | null) => {
    try {
      ...
      console.error('Failed to save trip id', error);
    }
  };
  ...
  const setTripStartTime = async (time: string | null) => {
    try {
      ...
      console.error('Failed to save trip start time', error);
    }
  };
